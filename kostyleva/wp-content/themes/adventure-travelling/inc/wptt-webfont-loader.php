...
if ( ! class_exists( 'Adventure_Travelling_WPTT_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Adventure_Travelling_WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.0.0
		 * @return array Returns an array of remote URLs and their local adventure_travelling_counterparts.
		 */
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $adventure_travelling_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $adventure_travelling_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$adventure_travelling_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$adventure_travelling_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$adventure_travelling_font_family = trim( str_replace( array( "'", ';' ), '', $adventure_travelling_font_family ) );
					$adventure_travelling_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $adventure_travelling_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $adventure_travelling_font_family ] ) ) {
					$result[ $adventure_travelling_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$adventure_travelling_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$adventure_travelling_font_family_url = $this->get_absolute_path( $adventure_travelling_font_family_url );

					$result[ $adventure_travelling_font_family ][] = $adventure_travelling_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $adventure_travelling_font_family ] = array_flip( array_flip( $result[ $adventure_travelling_font_family ] ) );
			}
			return $result;
		}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	...
	function wptt_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Adventure_Travelling_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'wptt_get_web_adventure_travelling_font_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_web_adventure_travelling_font_url( $url, $format = 'woff2' ) {
		$font = new Adventure_Travelling_WPTT_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
