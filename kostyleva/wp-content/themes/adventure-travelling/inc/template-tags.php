...
function adventure_travelling_categorized_blog() {
	$adventure_travelling_category_count = get_transient( 'adventure_travelling_categories' );

	if ( false === $adventure_travelling_category_count ) {
		// Create an array of all the categories that are attached to posts.
		$adventure_travelling_categories = get_categories( array(
			'fields'     => 'ids',
			'hide_empty' => 1,
		...
		$adventure_travelling_category_count = count( $adventure_travelling_categories );

		set_transient( 'adventure_travelling_categories', $adventure_travelling_category_count );
	}
	...
	return $adventure_travelling_category_count > 1;
}
